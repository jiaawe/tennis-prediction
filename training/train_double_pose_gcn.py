...
# GCN backbone for pose data
class PoseGCNBackbone(nn.Module):
    def __init__(self, num_keypoints=17, hidden_dim=64, output_dim=128):
        super(PoseGCNBackbone, self).__init__()
        ...
        self.projector = nn.Linear(hidden_dim * num_keypoints, output_dim)
...
# Dual Pose GCN model
class DualPoseGCN(nn.Module):
    def __init__(self, num_classes, num_keypoints=17, hidden_dim=64, feature_dim=128):
        super(DualPoseGCN, self).__init__()
        
        # Create two separate backbones for player and partner/future poses
        self.player_backbone = PoseGCNBackbone(num_keypoints, hidden_dim, feature_dim)
        self.second_backbone = PoseGCNBackbone(num_keypoints, hidden_dim, feature_dim)
    ...
    def forward(self, player_pose, second_pose):
        # Extract features from both poses
        player_features = self.player_backbone(player_pose)
        second_features = self.second_backbone(second_pose)
        
        # Concatenate features
        combined_features = torch.cat((player_features, second_features), dim=1)
...
# Training function
def train_epoch(model, dataloader, criterion, optimizer, device, second_type='partner'):
    model.train()
    running_loss = 0.0
    correct = 0
    total = 0
    ...
    for batch in tqdm(dataloader, desc="Training"):
        ...
        if second_type == 'partner':
            second_indices = batch['hitting_partner']
        else:  # 'future'
            second_indices = batch['hitting_player_n']
        
        # Get pose data
        poses = batch['poses']  # Shape: (batch_size, max_poses, 17, 3)
        
        # Extract the poses for each sample
        batch_size = poses.size(0)
        player_poses = []
        second_poses = []
        
        for i in range(batch_size):
            # Extract hitting player pose
            ...
            # Extract second pose (partner or future frame)
            second_idx = second_indices[i].item()
            if second_idx >= 0 and second_idx < poses.size(1):
                second_pose = poses[i, second_idx]
            else:
                # If invalid index, use zeros
                second_pose = torch.zeros(17, 3)
            second_poses.append(second_pose)
        
        # Stack into batch tensors
        player_poses = torch.stack(player_poses).to(device)  # Shape: (batch_size, 17, 3)
        second_poses = torch.stack(second_poses).to(device)  # Shape: (batch_size, 17, 3)
            
        # Zero the parameter gradients
        optimizer.zero_grad()
        
        # Forward pass
        outputs = model(player_poses, second_poses)
...
# Evaluation function
def evaluate(model, dataloader, criterion, device, second_type='partner', label_map=None, compute_metrics=False):
    model.eval()
    running_loss = 0.0
    correct = 0
    total = 0
...
if __name__ == '__main__':
    ...
    if label in ['outcome', 'shot_direction']:
        ...
        second_type = 'partner'
        print(f"Using partner pose for {label} prediction")
    
    # Create save directory with timestamp
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    save_dir = os.path.join('experiments', f'dual_gcn_{second_type}_run_{timestamp}_{label}')
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        train_loss, train_acc = train_epoch(
            model, train_loader, criterion, optimizer, device, second_type)
        
        # Validation
        if (epoch + 1) % 5 == 0:
            val_loss, val_acc, _ = evaluate(
                model, test_loader, criterion, device, second_type)
