...
#         else:
#             efficientnet = models.efficientnet_v2_m(weights=None)
        
#         # Extract the features (everything except the classifier)
#         # For EfficientNet, just remove the classifier
#         self.features = nn.Sequential(*list(efficientnet.children())[:-1])
        
#         # Get feature dimension (1280 for EfficientNet V2 M)
#         # This is the standard size for EfficientNet V2 M
...
#     def forward(self, x):
#         x = self.features(x)
#         # Properly handle the output of EfficientNet features
#         x = torch.flatten(x, 1)
#         x = self.classifier(x)
#         return x
